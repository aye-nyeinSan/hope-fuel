...
export async function POST(request) {
  ...
  try {
    ...
    if (transaction.length > 0) {
      ...
      const latestTransactionDate = new Date(latestTransaction.TransactionDate);
...
async function checkPermission(name, email) {
  ...
  if (Object.hasOwn(record, "notion_create_time")) {
    const lastestTranscation = new Date(record.notion_create_time);
    const lastestTranscationMonth = lastestTranscation.getMonth();
    const lastestTranscationYear = lastestTranscation.getFullYear();
  ...
  const lastestTranscation = new Date(record["Create Time"]);
  const lastestTranscationMonth = lastestTranscation.getMonth();
